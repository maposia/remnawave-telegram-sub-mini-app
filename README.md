...
| `CRYPTO_LINK`                    | Allows using encrypted links (currently supported Happ application). If no applications supporting cryptolink are present in `app-config.json`, these links will not be displayed                                                                                       |
...
| `AUTH_API_KEY`                   | If you use "Caddy with security" or TinyAuth for Nginx addon, place the X-Api-Key here. It will be applied to requests sent to the Remnawave Panel.                                                                                                                    |
| `TELEGRAM_BOT_TOKEN`             | Telegram bot token used to verify Mini App `initData` signatures on the backend. Required for secure access to subscription info.                                                                                                                                      |
| `TELEGRAM_INIT_DATA_TTL_SECONDS` | (Optional) Maximum age, in seconds, for accepted Telegram `initData`. Defaults to `300`. Increase only if your flow requires longer-lived init data.                                                                                                                   |
